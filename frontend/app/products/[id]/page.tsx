...
export default function ProductEditor() {
  ...
  if (!job || !form) {
    return (
      <Shell active="products" crumbs={["Acme Co.", "Products", id.slice(0, 8)]}>
    ...
    <Shell
      ...
      right={
        <div className="flex items-center gap-2">
          <span className="chip chip-accent hidden sm:inline-flex"><span className="dot" /> Saved locally</span>
          <button className="btn btn-sm hidden sm:inline-flex">Discard</button>
          <button className="btn-sm btn-primary">Save & sync</button>
        </div>
      }
    >
      <div className="flex-1 overflow-auto grid grid-cols-1 lg:grid-cols-2">
        <div className="px-4 md:px-8 py-5 md:py-7 border-b lg:border-b-0 lg:border-r border-line flex flex-col gap-5">
          <div>
            <div className="flex items-center gap-2.5 mb-1">
              <h1 className="text-[18px] md:text-[20px]">{form.title ?? "Untitled"}</h1>
        ...
        <div className="bg-[#F6F4EC] px-4 md:px-8 py-5 md:py-7 flex flex-col gap-4">
          <div className="flex items-center gap-2">
          ...
          <div className="bg-white rounded-lg border border-line overflow-hidden">
            <div className="h-7 border-b border-line bg-surface2 flex items-center px-3 font-mono text-[10.5px] text-muted truncate">
              {dest === "Shopify" ? "acme.myshopify.com" : dest === "Woo" ? "wp.acmehome.com" : "preview"}/products/{(form.sku || id).toLowerCase()}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 p-4 md:p-5 gap-4 md:gap-5">
              <div>
                {form.images?.[0] ? (
          ...
          {changes.length > 0 && (
            <div className="bg-white rounded-lg border border-line p-4">
              ...
              {changes.map((d, i) => (
                <div key={i} className={`grid gap-2.5 py-2 text-[12px] grid-cols-[70px_1fr] sm:grid-cols-[90px_1fr_auto] ${i === 0 ? "" : "border-t border-line-2"}`}>
                  ...
                  <span className="chip chip-ghost text-[10px] hidden sm:inline-flex">{d.reason}</span>
                </div>
              ))}
            </div>
          )}
